        ...
        <section className="card-form" style={{ maxWidth: 440, margin: "0 auto" }}>
          ...
          <form onSubmit={onLogin}>
            <div className="mb-3">
              ...
              <input className="form-control" value={usuarioLogin}
                onChange={(e) => setUsuarioLogin(e.target.value)} autoFocus />
            </div>
            <div className="mb-3">
              <label className="form-label">Contraseña</label>
              <input type="password" className="form-control" value={claveLogin}
...
function Usuarios({ onExpira }) {
  ...
  const [nuevo, setNuevo] = useState({ usuario: "", password: "", rol: "admin" });
    ...
    <>
      <section className="card-form">
        ...
        <form onSubmit={onCrear} className="row g-2 align-items-end">
          <div className="col-md-4">
            <label className="form-label">Usuario</label>
            <input className="form-control" value={nuevo.usuario}
              onChange={(e) => setNuevo({ ...nuevo, usuario: e.target.value })} />
          </div>
          <div className="col-md-4">
            <label className="form-label">Contraseña</label>
            <input type="text" className="form-control" value={nuevo.password}
              onChange={(e) => setNuevo({ ...nuevo, password: e.target.value })} />
          </div>
          <div className="col-md-2">
            <label className="form-label">Rol</label>
            <select className="form-control" value={nuevo.rol}
      ...
      <section className="card-form">
        ...
        <table className="table table-hover align-middle">
          <thead>
            <tr>
              <th>ID</th><th>Usuario</th><th>Rol</th><th>Estado</th><th className="text-end">Acciones</th>
            </tr>
          </thead>
          <tbody>
            {usuarios.length === 0 ? (
              <tr><td colSpan={5} className="text-center py-3 text-muted">Sin usuarios.</td></tr>
            ) : usuarios.map((u) => (
              <tr key={u.id}>
                <td>{u.id}</td>
                <td>{u.usuario}</td>
                <td>
                  <select className="form-select form-select-sm" style={{ width: 130 }}
                    value={u.rol} onChange={(e) => cambiarRol(u, e.target.value)}>
                    <option value="admin">admin</option>
                    <option value="developer">developer</option>
                  </select>
                </td>
                ...
                <td className="text-end table-actions">
                  <button className="btn btn-sm btn-outline-secondary me-2" onClick={() => resetearPassword(u)}>
                    Cambiar clave
                  </button>
                  <button className={`btn btn-sm ${u.activo ? "btn-outline-danger" : "btn-outline-success"}`}
...
function Logs({ onExpira }) {
  ...
  return (
    <section className="card-form">
      ...
      <form onSubmit={(e) => { e.preventDefault(); cargar(q); }} className="row g-2 mb-3">
        <div className="col">
          <input className="form-control" placeholder="Buscar por usuario, campo, acción o entidad"
            value={q} onChange={(e) => setQ(e.target.value)} />
        </div>
        <div className="col-auto">
          <button className="btn btn-primary" type="submit">Buscar</button>
      ...
      <table className="table table-sm table-hover align-middle">
        <thead>
          <tr>
            <th>Fecha</th><th>Usuario</th><th>Acción</th><th>Entidad</th><th>Campo</th>
            <th>Anterior</th><th>Nuevo</th>
          </tr>
        </thead>
        <tbody>
          {logs.length === 0 ? (
            ...
            <tr key={l.id}>
              <td style={{ whiteSpace: "nowrap" }}>{new Date(l.creado_en).toLocaleString("es-AR")}</td>
              <td>{l.usuario}</td>
              <td>{l.accion}</td>
              <td>{l.entidad} #{l.entidad_id}</td>
              <td>{l.campo}</td>
              <td style={{ maxWidth: 180, wordBreak: "break-word" }}>{l.valor_anterior}</td>
              <td style={{ maxWidth: 180, wordBreak: "break-word" }}>{l.valor_nuevo}</td>
            </tr>
          ))}
        </tbody>
      </table>
